...
ratioBeijing$City <- rep("Beijing",nrow(ratioBeijing))

ratioTianjin <- econData %>%
  mutate(OutputPerEnterprise = Tianjin_Output / Tianjin_Enterprise) %>%
  select(Year,OutputPerEnterprise)
ratioTianjin$City <- rep("Tianjin",nrow(ratioTianjin))

ratioHebei <- econData %>%
  mutate(OutputPerEnterprise = Hebei_Output / Hebei_Enterprise) %>%
  select(Year,OutputPerEnterprise)
ratioHebei$City <- rep("Hebei",nrow(ratioHebei))

ratioShanghai <- econData %>%
  mutate(OutputPerEnterprise = Shanghai_Output / Shanghai_Enterprise) %>%
  select(Year,OutputPerEnterprise)
ratioShanghai$City <- rep("Shanghai",nrow(ratioShanghai))

# Combines all the data frames into one for plotting
ratio <- rbind(ratioBeijing, ratioTianjin, ratioHebei, ratioShanghai) %>%
  filter(Year >= 2000)

# Plots the data
ggplot(data=ratio,aes(x=Year,y=OutputPerEnterprise,color=City)) +geom_line() + ylab("Output per # of Enterprises") + ggtitle("Output per Enterprise of Various Chinese Cities") +theme_bw()

```
We can see a few trends in this plot:
First, a tight linear growth in which most of the cities are just about equal and the lead spot goes from Shanghai to Hebei around 2005.
